...
def LSTMCNN(opt):
    ...
    for l in range(opt.num_layers):
        ...
        # if opt.RNN:
        #     x = LSTM(opt.rnn_size, activation=opt.LSTMactivation, recurrent_activation=opt.LSTMrec_activation, return_sequences=True, stateful=True)(x)

        # else:
        #     x = GRU(opt.rnn_size, activation=opt.GRUactivation, recurrent_activation=opt.GRUrec_activation, return_sequences=True, stateful=True)(x)
    ...
    if opt.optimizer:
        optimizer = RMSprop(lr=opt.learning_rateRMS, rho=opt.rhoRMS, epsilon=opt.epsilonRMS, decay=opt.learning_rate_decayRMS)
    else:
