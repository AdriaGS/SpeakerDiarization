...
class BatchLoaderUnk:
    def __init__(self, tokens, data_dir, batch_size, seq_length, max_word_l, n_words, n_chars, delay, vector_size):
        ...
        for split, data in enumerate(all_data):
            ...
            p_batches = np.split(rprb, rprb.shape[1]/seq_length, axis=1) if vector_size > 0 else None
    ...
    def next_batch(self, split_idx):
        while True:
            ...
            prb = self.all_batches[split_idx][4][idx] if self.all_batches[split_idx][4] is not None else None
            # expand dims
